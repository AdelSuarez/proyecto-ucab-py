...
class Screen_state:
    ...
    def screen_victory(self, screen, moves):
        ...
        screen.blit(victory_img, victory_img.get_rect(
            center=(st.SCREEN_WIDTH/2, 250)))
        Text.Text(f'-Movimientos realizados: {moves}-',
                  self._font_state, st.WHITE).draw_text_center(screen, 750)
    ...
    def screen_game_over(self, screen, state=None):
        ...
        Text.Text('Game Over', self._font_game_over,
                  st.WHITE,).draw_text_center(screen, 200)

        if state == '#':
            Text.Text('-Robot destruido-', self._font_state,
                      st.WHITE).draw_text_center(screen, 350)
        else:
            Text.Text('-Movimientos agotados-', self._font_state,
                      st.WHITE).draw_text_center(screen, 350)
    ...
    def screen_pause(self, screen):
        ...
        Text.Text('Pause menu', self._font_pause_menu,
                  st.WHITE).draw_text_center(screen, 120)
