...
class Screen_game:
    ...
    def __init__(self, game_over, address, map_game, game_pause) -> None:
        self._font = pygame.font.Font("assets/font.ttf", 15)
    ...
    def game(self, screen):
        while not self.game_over:
            ...
            if Collision_checker(self.map_game).checker() == '#' or Screen_game.counter_move == 41:
                Screen_state(asset.BG_opacity, self.game_over).game_over(screen)

            elif  Collision_checker(self.map_game).checker() == '@':
                Screen_state(asset.BG_opacity, self.game_over, asset.victory_img).victory(screen)
