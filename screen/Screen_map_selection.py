...
class Screen_map_selection:
    ...
    def screen_selection_map(self, screen) -> str:
        Text.Text('Map', self._font_controller,
                  st.WHITE).draw_text_center(screen, 100)

        Text.Text(
            'Random Map',
            self._font_text_title,
            st.WHITE
        ).draw_text(screen, 150, 300)
        Text.Text(
            'txt Map',
            self._font_text_title,
            st.WHITE
        ).draw_text(screen, 730, 300)
        ...
        Text.Text(
            ...
            self._font_text,
            st.WHITE
        ).draw_text(screen, 670, 500)

        Text.Text(
            'the txt map can not be used the keys, it is only to show the result of the data',
            self._font_text,
            st.WHITE
        ).draw_text_center(screen, 700)
